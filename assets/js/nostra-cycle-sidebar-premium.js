/* Grupo Nostradamus - Panel lateral premium para ciclos UNI */
(function () {
  var file = (location.pathname.split('/').pop() || '').toLowerCase();
  var cycles = {
    'ciclo-anual-uni.html': {
      name: 'Nostra 360 UNI',
      tag: 'Preparación integral. Formación completa.',
      icon: '🌐'
    },
    'ciclo-semianual-uni.html': {
      name: 'Nostra Power UNI',
      tag: 'Corrige tus errores. Potencia tu nivel.',
      icon: '⚡'
    },
    'ciclo-semestral-uni.html': {
      name: 'Nostra Élite UNI',
      tag: 'Estuviste cerca. Ahora ve por tu vacante.',
      icon: '🎯'
    },
    'ciclo-repaso-uni.html': {
      name: 'Nostra Prime UNI',
      tag: 'Tu máximo nivel. Tu momento decisivo.',
      icon: '🚀'
    },
    'ciclo-elite-uni.html': {
      name: 'Nostra Talentum UNI',
      tag: 'Talento superior. Exigencia máxima.',
      icon: '🏆'
    }
  };
  ...
  function addStyle() {
    ...
    style.textContent = `
      .nostra-cycle-panel {
        ...
        border-radius: 28px;
        overflow: hidden;
        background: radial-gradient(circle at 12% 12%, rgba(0, 229, 255, .18), transparent 34%), linear-gradient(155deg, #02070d 0%, #061426 46%, #063a48 100%);
        border: 1px solid rgba(168, 247, 255, .24);
        box-shadow: 0 28px 70px rgba(0, 0, 0, .18), 0 0 34px rgba(0, 194, 209, .12);
        color: #fff;
        padding: 24px;
      }
      .nostra-cycle-panel__badge {
        display: inline-flex;
        align-items: center;
        gap: 8px;
        padding: 8px 13px;
        ...
        border: 1px solid rgba(168, 247, 255, .26);
        color: #a8f7ff;
        font-weight: 900;
        font-size: 13px;
      ...
      .nostra-cycle-panel__trust {
        display: grid;
        grid-template-columns: 1fr;
        gap: 9px;
        margin: 18px 0 20px;
      }
      .nostra-cycle-panel__trust span {
        display: flex;
        align-items: center;
        gap: 9px;
        padding: 10px 12px;
      ...
      @media (max-width: 991px) {
        .nostra-cycle-panel { position: relative; top: auto; margin-top: 28px; }
      }
    `;
  ...
  function buildPanel() {
    ...
    sidebar.innerHTML = `
      <div class="nostra-cycle-panel">
        <span class="nostra-cycle-panel__badge">${data.icon} Ruta premium UNI</span>
