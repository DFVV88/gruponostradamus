/* ==================================================
   NostraCHAT v2.0 - núcleo optimizado para Blaze
   Un solo listener de mensajes por usuario y sala.
================================================== */
(function () {
  var firebaseConfig = window.NOSTRACHAT_FIREBASE_CONFIG;
  var maxMessageLength = 420;
  var MESSAGE_LIMIT = 40;
  var MESSAGE_RETENTION_DAYS = 30;
  var REPORT_RETENTION_DAYS = 90;
  ...
  function looksLikeSpam(text) {
    ...
    var words = t.split(' ');
    return words.length >= 6 && Array.from(new Set(words)).length <= 2;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .nchat-title p{font-size:18px;line-height:1.65;color:#4b5d70;margin:0}.nchat-shell{display:grid;grid-template-columns:320px minmax(0,1fr) 250px;gap:20px;align-items:stretch}
      ...
      .nchat-online-card{padding:18px;display:flex;flex-direction:column;min-height:650px}.nchat-online-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px}.nchat-online-head h3{margin:0;color:#061426;font-size:22px;font-weight:950}.nchat-online-count{display:inline-flex;align-items:center;justify-content:center;min-width:32px;height:32px;border-radius:999px;background:linear-gradient(135deg,#078c95,#061426);color:#fff;font-weight:950;font-size:13px}.nchat-online-sub{font-size:12px;color:#687789;line-height:1.35;margin-bottom:12px;font-weight:800}.nchat-online-list{display:flex;flex-direction:column;gap:9px;overflow:auto;max-height:560px;padding-right:3px}.nchat-online-item{border:1px solid rgba(7,140,149,.15);background:#f9fdff;border-radius:16px;padding:10px 11px}.nchat-online-name{display:flex;align-items:center;gap:7px;font-weight:950;color:#061426;font-size:14px}.nchat-online-dot{width:9px;height:9px;border-radius:999px;background:#13c45e;box-shadow:0 0 0 4px rgba(19,196,94,.12)}.nchat-online-extra{font-size:12px;color:#617184;margin-top:5px}.nchat-online-empty{font-size:13px;color:#69788a;line-height:1.4;font-weight:800;padding:18px 4px;text-align:center}
      @media(max-width:1100px){.nchat-shell{grid-template-columns:300px minmax(0,1fr)}}@media(max-width:991px){.nchat-shell{grid-template-columns:1fr}.nchat-online-card{min-height:auto;order:3}.nchat-online-list{max-height:220px}.nchat-main{min-height:560px}.nchat-composer{flex-direction:column}.nchat-send{min-height:46px}.nchat-msg{max-width:94%}}
    `;
  ...
  function buildUI() {
    ...
    section.innerHTML = '<div class="nchat-wrap"><div class="nchat-title"><span>Salas oficiales activas</span><h2>Entrar al chat académico</h2><p>NostraCHAT se conecta con tu NostraCUENTA y conserva solo el historial reciente para funcionar de forma rápida y eficiente.</p></div>' +
      ...
      '<main class="nchat-card nchat-main"><div class="nchat-head"><div><strong id="nchat-room-title-main">NostraCHAT · Alumnos · General</strong><span id="nchat-room-subtitle">Verificando NostraCUENTA...</span></div><div class="nchat-pill" id="nchat-status">Verificando</div></div><div class="nchat-messages" id="nchat-messages"><div class="nchat-empty">Ingresa para ver y enviar mensajes.</div></div><div class="nchat-composer"><textarea id="nchat-message" maxlength="420" placeholder="Escribe tu consulta académica..." disabled></textarea><button class="nchat-send" id="nchat-send" disabled>Enviar</button></div><div class="nchat-note">Se muestran los 40 mensajes más recientes. El historial antiguo se elimina automáticamente.</div></main>' +
      '<aside class="nchat-card nchat-online-card" id="nchat-online-panel"><div class="nchat-online-head"><h3>Conectados</h3><span class="nchat-online-count" id="nchat-online-count">0</span></div><div class="nchat-online-sub" id="nchat-online-sub">Personas activas en esta sala.</div><div class="nchat-online-list" id="nchat-online-list"><div class="nchat-online-empty">Ingresa al chat para ver usuarios conectados.</div></div></aside></div></div>';
  ...
  function initChat() {
    ...
    Promise.all([
      ...
      var state = {
        ...
        listeningRoomId: '', lastSendAt: 0, lastMessageText: '', latestItems: [],
      ...
      function expiry(days) { return fs.Timestamp.fromMillis(Date.now() + Number(days) * 86400000); }
      ...
      function emitMessages(items) {
        state.latestItems = items;
        window.dispatchEvent(new CustomEvent('nostrachat:messages', { detail: {
          roomId: state.roomId, zone: state.zone, items: items, db: db, fs: fs,
          user: state.user, sessionId: state.sessionId, roomLabel: getRoomLabel(state.roomId)
        }}));
      }
      window.NOSTRACHAT_RUNTIME = { db: db, fs: fs, auth: auth, state: state, expiry: expiry, getRoomLabel: getRoomLabel };
      window.dispatchEvent(new CustomEvent('nostrachat:runtime-ready', { detail: window.NOSTRACHAT_RUNTIME }));
      ...
      function enableChat() {
        ...
        status.textContent = document.hidden ? 'En pausa' : 'Conectado';
        updateTitle();
      }
      function updateAccessUI() {
        ...
        if (isAlumno && !state.user) showWarning('La zona alumnos usa <b>NostraCUENTA</b>. Primero ingresa en NostraCUENTA y vuelve a NostraCHAT.', 'info');
      ...
      function renderMessages(items) {
        ...
        messages.innerHTML = items.map(function (m) {
          ...
          return '<div class="nchat-msg ' + cls + reported + '" data-message-id="' + id + '"><div class="nchat-meta"><span>' + escapeHTML(m.name || 'NostraCHAT') + '</span><span>' + escapeHTML(time) + '</span></div><div class="nchat-text">' + escapeHTML(m.text || '') + '</div><div class="nchat-actions"><button class="nchat-report" data-report-id="' + id + '"' + reportDisabled + '>' + reportText + '</button></div></div>';
        }).join('');
      ...
      function startListening() {
        ...
        if (state.unsubscribe && state.listeningRoomId === state.roomId) return;
        stopListening();
        state.listeningRoomId = state.roomId;
        messages.innerHTML = '<div class="nchat-empty">Cargando mensajes recientes...</div>';
        var q = fs.query(fs.collection(db, roomPath()), fs.orderBy('createdAt', 'desc'), fs.limit(MESSAGE_LIMIT));
        state.unsubscribe = fs.onSnapshot(q, function (snapshot) {
          ...
          messages.innerHTML = '<div class="nchat-empty">No se pudieron cargar mensajes. Verifica tu acceso.</div>';
      ...
      form.addEventListener('submit', function (e) {
        ...
        var name = cleanText(nameInput.value), extra = cleanText(extraInput.value);
      ...
      function sendMessage() {
        ...
        fs.addDoc(fs.collection(db, roomPath()), {
          text: text, name: state.user.name, extra: state.user.extra, zone: state.zone,
          roomId: state.roomId, roomLabel: getRoomLabel(state.roomId), sessionId: state.sessionId,
          type: 'message', moderationStatus: 'visible', pinned: false,
          createdAt: fs.serverTimestamp(), expiresAt: expiry(MESSAGE_RETENTION_DAYS)
        }).then(function () { messageInput.value = ''; hideWarning(); })
          .catch(function (err) { showWarning('No se pudo enviar. Verifica tu acceso o las reglas de Firestore.'); console.error(err); })
          .finally(function () { sendBtn.disabled = false; messageInput.focus(); });
      }
      function reportMessage(messageId) {
        if (!state.user) return showWarning('Primero debes ingresar al chat.');
        if (!messageId || state.reportedIds[messageId]) return;
        var reason = cleanText(window.prompt('¿Por qué reportas este mensaje? Ej: insulto, spam, contenido no académico') || 'Reporte del usuario');
        if (reason.length < 3) reason = 'Reporte del usuario';
        fs.addDoc(fs.collection(db, reportsPath()), {
          messageId: messageId, room: state.zone, roomId: state.roomId, roomLabel: getRoomLabel(state.roomId),
          reportedBy: state.user.name, reporterExtra: state.user.extra, reporterSessionId: state.sessionId,
          reason: reason.slice(0,180), status: 'pendiente', createdAt: fs.serverTimestamp(), expiresAt: expiry(REPORT_RETENTION_DAYS)
        }).then(function () {
          state.reportedIds[messageId] = true;
          localStorage.setItem('nostrachat_reported_ids', JSON.stringify(state.reportedIds));
          var btn = messages.querySelector('[data-report-id="' + CSS.escape(messageId) + '"]');
          if (btn) { btn.disabled = true; btn.textContent = 'Reportado'; var item = btn.closest('.nchat-msg'); if (item) item.classList.add('reported'); }
          showWarning('Reporte enviado. Coordinación podrá revisarlo.', 'info');
        }).catch(function (err) { showWarning('No se pudo enviar el reporte.'); console.error(err); });
      }
      ...
      document.addEventListener('visibilitychange', function () {
        if (document.hidden) {
          stopListening();
          if (state.user) status.textContent = 'En pausa';
        } else if (state.user) {
      ...
      try {
        ...
        if (savedExt && savedExt.name && savedExt.extra && state.zone === 'externos') {
          state.user = savedExt; nameInput.value = savedExt.name; extraInput.value = savedExt.extra; rulesInput.checked = true; enableChat(); startListening();
        }
      } catch (e) {}
