...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      #nostrachat-live,
      #nostrachat-live *{box-sizing:border-box;}
      ...
      .nchat-title{text-align:center;max-width:880px;margin:0 auto 34px;}
      ...
      .nchat-shell{display:grid;grid-template-columns:340px 1fr;gap:20px;align-items:stretch;}
      ...
      .nchat-zone{display:flex;align-items:center;gap:10px;width:100%;padding:14px 15px;border:1px solid rgba(7,140,149,.16);background:#f7fdff;border-radius:16px;margin:10px 0;cursor:pointer;font-weight:950;color:#061426;text-align:left;}
      ...
      .nchat-form input,.nchat-form select{width:100%;border:1px solid rgba(7,140,149,.22);border-radius:14px;padding:12px 13px;font:inherit;outline:none;background:#fff;}
      ...
      .nchat-btn:disabled{opacity:.55;cursor:not-allowed;box-shadow:none;}
      ...
      .nchat-main{display:flex;flex-direction:column;min-height:620px;}
      ...
      <div class="nchat-wrap">
        ...
        <div class="nchat-shell">
          <aside class="nchat-card nchat-side">
            <h3>Acceso</h3>
            <p>Elige tu zona, identifícate y acepta las reglas. Todo mensaje debe ser académico y respetuoso.</p>
          ...
          <main class="nchat-card nchat-main">
            <div class="nchat-head">
              <div><strong id="nchat-room-title">NostraCHAT · Alumnos</strong><span id="nchat-room-subtitle">Ingresa tus datos para empezar</span></div>
  ...
  function initChat() {
    ...
    import('https://www.gstatic.com/firebasejs/10.12.5/firebase-app.js').then(function (appMod) {
      ...
      var title = document.getElementById('nchat-room-title');
      ...
      function setZone(zone) {
        ...
        title.textContent = zone === 'alumnos' ? 'NostraCHAT · Alumnos' : 'NostraCHAT · Externos';
      ...
      function enableChat() {
        ...
        subtitle.textContent = 'Participas como ' + state.user.name;
      }
      ...
      zoneButtons.forEach(function (btn) {
        btn.addEventListener('click', function () { setZone(btn.dataset.zone); });
      });
      ...
      function sendMessage() {
        ...
        fs.addDoc(fs.collection(db, roomPath()), {
          ...
          showWarning('No se pudo enviar. Revisa la configuración de Firestore.');
          console.error(err);
        }).finally(function () {
          sendBtn.disabled = false;
          messageInput.focus();
