/* ==================================================
   Grupo Nostradamus - Preparación de preinscripción para Culqi
   Lee el tarifario oficial, muestra el resumen de compra,
   exige aceptación legal y guarda una fotografía histórica.
================================================== */
(function(){
  'use strict';
  ...
  function formValue(form,name){
    return clean(form.elements[name] && form.elements[name].value);
  }
  ...
  function addStyles(){
    ...
    style.textContent = '\
      ...
      .npc-summary.is-visible{display:block}\
      .npc-summary__head{padding:17px 19px;background:linear-gradient(135deg,#061426,#07515a,#078c95);color:#fff}\
      ...
      .npc-summary__head strong{display:block;font-family:"Baloo 2",Jost,Arial,sans-serif;font-size:25px;line-height:1.05}\
      .npc-summary__body{padding:18px 19px}\
      .npc-summary__grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:11px}\
      .npc-summary__item{padding:12px 13px;border-radius:14px;background:#f4fbfc;border:1px solid rgba(7,140,149,.13)}\
      .npc-summary__item span{display:block;margin-bottom:3px;color:#607080;font-size:10px;font-weight:950;text-transform:uppercase;letter-spacing:.35px}\
      .npc-summary__item b{display:block;color:#061426;font-size:15px;line-height:1.3}\
      .npc-summary__total{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-top:13px;padding:16px;border-radius:17px;background:#061426;color:#fff}\
      .npc-summary__total span{font-size:13px;font-weight:850}.npc-summary__total strong{color:#62e5eb;font-size:28px;line-height:1}\
      .npc-summary__after{margin:12px 0 0;color:#4b5d70;font-size:13px;line-height:1.5;font-weight:750}\
      .npc-summary__status{padding:16px 19px;color:#075b65;font-size:13px;font-weight:850}\
      .npc-summary__status.error{color:#b42318;background:#fff2f2}\
      .npc-legal{grid-column:1/-1;margin-top:4px;padding:18px;border-radius:20px;background:#f5fbfc;border:1px solid rgba(7,140,149,.2)}\
      .npc-legal label{display:grid;grid-template-columns:auto 1fr;gap:11px;align-items:start;cursor:pointer}\
      .npc-legal input{width:20px!important;height:20px;margin-top:2px;accent-color:#078c95}\
      .npc-legal strong{display:block;color:#061426;font-size:15px;line-height:1.4}\
      .npc-legal p{margin:7px 0 0;color:#5f6b7a;font-size:13px;line-height:1.55}\
      .npc-legal a{color:#007b86;font-weight:900;text-decoration:underline}\
  ...
  function renderOfficial(official){
    ...
    box.innerHTML = '<div class="npc-summary__head"><small>Resumen oficial de compra</small><strong>' + esc(official.program.nombre) + ' · ' + esc(official.plan.nombre) + '</strong></div>' +
      ...
      '<div class="npc-summary__item"><span>Concepto inicial</span><b>' + esc(official.calc.conceptoInicial) + '</b></div>' +
      promo +
      ...
      '</div><div class="npc-summary__total"><span>Total que se registrará para el pago inicial</span><strong>' + money(official.calc.totalInicial) + '</strong></div>' +
      '<p class="npc-summary__after"><b>Pagos posteriores:</b> ' + esc(official.detallePagosPosteriores) + '<br>El backend volverá a consultar este tarifario antes de efectuar el cobro con Culqi.</p></div>';
  }
  function officialSelection(form){
    ...
    return initFirebase().then(function(ctx){
      return ctx.fs.getDoc(ctx.fs.doc(ctx.db,'programas_publicos',programId)).then(function(snapshot){
        ...
        var programName = clean(program.nombre) || selectedText(form.elements.ciclo);
        var modality = clean(plan.modalidad) || inferModalidad(plan.nombre);
        var shift = clean(plan.turno) || inferTurno(plan.nombre);
        return {
          ctx:ctx,
          program:{id:programId,nombre:programName,fechaInicio:clean(program.fechaInicio),duracion:clean(program.duracion),esquemaPrecios:num(program.esquemaPrecios) || 3},
          plan:plan,
          modalidad:modality,
          turno:shift,
          calc:calc,
  ...
  function refreshSummary(form){
    ...
    officialSelection(form).then(function(official){
      ...
      if(requestId !== summaryRequest) return;
      renderUnavailable(error.message || 'No se pudo consultar el tarifario oficial.');
    });
  }
  ...
  function validateBasic(form){
    ...
    if(!formValue(form,'metodoPagoPreferido')) return 'Selecciona una forma de pago.';
    if(!formValue(form,'confirmacion')) return 'Acepta que Coordinación se comunique contigo.';
    if(!document.getElementById('acepta-condiciones-compra')?.checked) return 'Debes aceptar los términos, la política de cambios y la política de privacidad.';
    return '';
  }
  ...
  function buildData(form,official,refId){
    var method = formValue(form,'metodoPagoPreferido');
  ...
  function voucherButtonHtml(data,id){
    ...
    return '<br><br><a href="' + url + '" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:13px 18px;background:linear-gradient(135deg,#25d366,#078c95);color:#fff;font-weight:950;text-decoration:none;">📲 Enviar voucher por WhatsApp</a><br><small>Adjunta la imagen del voucher en ese mismo chat.</small>';
  }
  function submit(form){
    var error = validateBasic(form);
    if(error){ message(form,'error',error); return; }
    var button = form.querySelector('button[type="submit"]');
    if(button){ button.disabled = true; button.textContent = 'Verificando tarifario oficial...'; }
    message(form,'info','Verificando el plan y el monto oficial antes de registrar la solicitud...');

    officialSelection(form).then(function(official){
      var ctx = official.ctx;
      var ref = ctx.fs.doc(ctx.fs.collection(ctx.db,'preinscripciones'));
      var data = buildData(form,official,ref.id);
      data.createdAt = ctx.fs.serverTimestamp();
      data.updatedAt = ctx.fs.serverTimestamp();
      return ctx.fs.setDoc(ref,data).then(function(){ return {ref:ref,data:data}; });
    }).then(function(result){
      var data = result.data;
      var extra = data.metodoPagoPreferido === 'voucher_whatsapp'
        ? '<br>Forma elegida: voucher por WhatsApp. La matrícula solo se aprobará después de validar el pago.' + voucherButtonHtml(data,result.ref.id)
        : '<br>Forma elegida: pago en línea. La solicitud quedó preparada para continuar con Culqi cuando se habilite el checkout.';
      message(form,'ok','✅ Preinscripción registrada correctamente.<br><small>Código: ' + esc(data.codigoSolicitud) + '</small><br><b>Pago inicial registrado: ' + money(data.totalInicial) + '</b>' + extra);
      form.reset();
      currentOfficial = null;
      renderEmpty();
      if(form.elements.ciclo) form.elements.ciclo.dispatchEvent(new Event('change',{bubbles:true}));
      if(typeof gtag === 'function'){
        gtag('event','preinscripcion_tarifario_validado',{
          event_category:'lead',
          event_label:data.ciclo + ' - ' + data.planNombre,
          metodo_pago:data.metodoPagoPreferido,
          value:data.totalInicial,
          currency:'PEN'
        });
      }
    }).catch(function(saveError){
      console.error('No se pudo registrar la preinscripción con tarifario:',saveError);
      message(form,'error',saveError?.code === 'permission-denied'
        ? 'Firebase no permitió registrar la solicitud. Deben revisarse las reglas de preinscripciones.'
        : (saveError.message || 'No se pudo verificar el tarifario o guardar la solicitud.'));
    }).finally(function(){
