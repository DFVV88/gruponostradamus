/* ==================================================
   NostraCHAT - Acceso institucional v1
   Microsoft 365 verifica @gruponostradamus.edu.pe.
   Luego se crea usuario corto + contraseña.
================================================== */
(function () {
  ...
  function internalEmail(username){ return userKey(username) + INTERNAL_DOMAIN; }
  function validUsername(u){ return /^[a-z0-9._]{5,20}$/.test(u) && reserved.indexOf(u) === -1; }
  function validPass(p){ return /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/.test(p || ''); }
  function validInstitutionalEmail(email){ return String(email || '').toLowerCase().endsWith(DOMAIN); }
  function html(v){ return String(v || '').replace(/[&<>'"]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c];}); }

  function isAlumnoZone(){
  ...
  function buildPanel(){
    ...
    panel.innerHTML = `
      <h4>🔐 Acceso alumnos y docentes</h4>
      <p>Primero verifica tu correo Microsoft 365 institucional. Luego crea tu usuario corto y contraseña para ingresar a NostraCHAT.</p>
      <div class="nchat-auth-tabs">
        <button type="button" class="nchat-auth-tab active" data-auth-tab="login">Ingresar</button>
        <button type="button" class="nchat-auth-tab" data-auth-tab="register">Registrarme</button>
      </div>
      <div class="nchat-auth-view show" id="nchat-auth-login-view">
        <label class="nchat-auth-field"><span>Usuario corto</span><input id="nchat-login-username" maxlength="20" placeholder="Ej. juanperez" autocomplete="username"></label>
        <label class="nchat-auth-field"><span>Contraseña</span><input id="nchat-login-password" type="password" placeholder="Tu contraseña" autocomplete="current-password"></label>
        <button type="button" class="nchat-auth-btn" id="nchat-login-btn">Ingresar</button>
      </div>
      <div class="nchat-auth-view" id="nchat-auth-register-view">
        <button type="button" class="nchat-auth-btn secondary" id="nchat-ms-verify-btn">✅ Paso 1: verificar con Microsoft 365</button>
        <div class="nchat-auth-profile" id="nchat-ms-profile"></div>
        <div id="nchat-register-fields" style="display:none">
          <div class="nchat-auth-row">
            <label class="nchat-auth-field"><span>Usuario corto</span><input id="nchat-reg-username" maxlength="20" placeholder="Ej. juanperez"></label>
            <label class="nchat-auth-field"><span>Rol</span><select id="nchat-reg-role"><option value="student">Alumno</option><option value="teacher">Docente</option></select></label>
          </div>
          <label class="nchat-auth-field"><span>Nombre completo</span><input id="nchat-reg-name" maxlength="80" placeholder="Nombre y apellido"></label>
          <label class="nchat-auth-field"><span>Ciclo, código WTC o curso principal</span><input id="nchat-reg-detail" maxlength="80" placeholder="Ej. Semianual UNI / WTC-001 / Física"></label>
          <div class="nchat-auth-row">
            <label class="nchat-auth-field"><span>Contraseña</span><input id="nchat-reg-password" type="password" autocomplete="new-password" placeholder="Mín. 8, mayúscula y número"></label>
            <label class="nchat-auth-field"><span>Confirmar contraseña</span><input id="nchat-reg-password2" type="password" autocomplete="new-password" placeholder="Repite la contraseña"></label>
          </div>
          <button type="button" class="nchat-auth-btn" id="nchat-create-account-btn">Crear cuenta NostraCHAT</button>
        </div>
      </div>
      <div class="nchat-auth-message" id="nchat-auth-message"></div>
      <div class="nchat-auth-profile" id="nchat-active-profile"></div>
    `;
  ...
  function unlockChat(profile){
    ...
    var localUser = {
      ...
      extra: profile.roleLabel + ' · ' + profile.username + ' · ' + profile.institutionalEmail,
      zone: 'alumnos',
      uid: profile.uid,
      role: profile.role,
    ...
    var name = document.getElementById('nchat-name');
    var extra = document.getElementById('nchat-extra');
    var rules = document.getElementById('nchat-rules');
    var form = document.getElementById('nchat-form');
    ...
    if(box){
      ...
      box.innerHTML = '<strong>✅ Sesión activa: ' + html(profile.name || profile.username) + '</strong><small>Usuario: ' + html(profile.username) + ' · ' + html(profile.roleLabel) + '</small><button type="button" class="nchat-auth-link" id="nchat-logout-btn">Cerrar sesión</button>';
  ...
  function verifyMicrosoft(){
    ...
    authMod.signInWithPopup(auth, provider).then(function(result){
      ...
      if(prof){
        ...
        prof.innerHTML = '<strong>Correo verificado</strong><small>' + html(email) + '</small>';
      }
      ...
      console.error(err);
      msg('error','No se pudo verificar con Microsoft. Revisa que el proveedor Microsoft esté habilitado en Firebase Authentication.');
    });
  }

  function createAccount(){
    var username = userKey(document.getElementById('nchat-reg-username') && document.getElementById('nchat-reg-username').value);
    ...
    fs.getDoc(unameRef).then(function(snap){
      ...
      var profile = {
        ...
        name: name,
        role: role,
        roleLabel: role === 'teacher' ? 'Docente' : 'Alumno',
        detail: detail,
        status: 'pending',
        blocked: false,
        createdAt: fs.serverTimestamp()
      };
      return fs.setDoc(fs.doc(db, 'users/' + uid), profile).then(function(){
        return fs.setDoc(unameRef, { uid: uid, institutionalEmail: verified.email, createdAt: fs.serverTimestamp() });
      });
    }).then(function(){
      ...
      msg('error','No se pudo crear la cuenta. Revisa Firebase Auth, reglas de Firestore o intenta con otro usuario.');
    });
  }

  function login(){
    var username = userKey(document.getElementById('nchat-login-username') && document.getElementById('nchat-login-username').value);
  ...
  function bindEvents(){
    ...
    var ms = document.getElementById('nchat-ms-verify-btn');
    var create = document.getElementById('nchat-create-account-btn');
    var loginBtn = document.getElementById('nchat-login-btn');
  ...
  function removeLegacyAlumnoSession(){
    try{
    ...
    var timer = setInterval(function(){
      ...
      if(buildPanel()){
        ...
        initFirebase().then(function(){
          ...
          setTab('login');
          updateMode();
        }).catch(function(err){ console.error(err); msg('error','No se pudo cargar Firebase Auth.'); });
      }
